...
def accuracy(testX, testY, a, b, scaler=None):
    ...
    testLen=float(len(testY))
    sumX = sum(testY*(testX.dot(a)+b)>0)
    print(sumX)
    print (testLen)
    ratio = (sumX/testLen)
    print(ratio)
...
def update(a, b, X, Y, e, l, acc, testX, testY):
    ...
    for i in range(len(Y)):
        ...
        if i % 30 == 0:
            ...
            #print("accuracy")
            #print(accuracy(testX, testY, a, b))
    return a, b
...
#           epoch_fun=lambda x: 1/(0.05*x+100)):
    ...
    print("train using the dumb thing")    
    print( a, b,loss,acc)    
    return (a, b, loss, acc)
...
if __name__ == "__main__":
    ...
    X = np.array(range(1, len(acc1)))
    p1, = pyplot.plot(X, acc1[1:len(acc1)],label='lambda=1')
    p2, = pyplot.plot(X, acc2[1:len(acc2)],label='lambda=0.1')
    p3, = pyplot.plot(X, acc3[1:len(acc3)],label='lambda=0.01')
    p4, = pyplot.plot(X, acc4[1:len(acc4)],label='lambda=0.001')
    pyplot.legend(handles=[p1,p2,p3,p4], bbox_to_anchor=(0., 1.02, 1., .102), loc=3,
           ncol=2, mode="expand", borderaxespad=0.)
    #pyplot.legend(handles=[p1,p2], bbox_to_anchor=(0., 1.02, 1., .102), loc=3,
     #      ncol=2, mode="expand", borderaxespad=0.)
    pyplot.show()
